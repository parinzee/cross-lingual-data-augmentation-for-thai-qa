...
def get_offset_begin_position(cleaned_context, answers_text, answer_begin_positions):
    ...
    try:
        ...
        logger.critical("Error with answer begin position: %s", str(answer_begin_position))
        raise
...
def generate_id(row):
    return hashlib.sha256((row["context"] + row["question"]).encode("utf-8")).hexdigest()
...
def load_unprocessed_qa_dataset():
    ...
    # Save dataset
    logger.info("Saving dataset to data/01_unprocessed_qa_dataset.csv")
    final.to_csv("data/01_unprocessed_qa_dataset.csv", index=False)
    logger.info("Saved dataset to data/01_unprocessed_qa_dataset.csv")
